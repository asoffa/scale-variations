...
from config import USE_TRUTH_JETS
...
def get_SRs(opt=""):
    ...
    if opt=="sr0b2_j25":
        SRs[    "SR0b2_j25"] = n_lep_cut(2, is_reco=False) + " && {} == 0 && {} >= 6 && met > 150 && meff >  900".format(n_bjets_20, n_jets_25)
        return SRs

    raise Exception("regions.get_SRs: invalid option: {}".format(opt))


def get_reco_SRs(rpv=False, sr0b2_j25=False):
    if opt=="":
    ...
    if opt=="sr0b2_j25":
        SRs_reco["SR0b2_j25"] = n_lep_cut(2, is_reco=True ) + " && n_bjets_20 == 0 && n_jets_25 >= 6 && met > 150 && meff >  900"
        return SRs_reco

    raise Exception("regions.get_reco_SRs: invalid option: {}".format(opt))
